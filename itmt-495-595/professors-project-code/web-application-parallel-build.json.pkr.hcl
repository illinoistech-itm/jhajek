...
build {
  sources = ["source.virtualbox-iso.lb", "source.virtualbox-iso.ws1", "source.virtualbox-iso.ws2", "source.virtualbox-iso.ws3"]
  ...
  provisioner "shell" {
    environment_vars = ["LBIP={{user `load-balancer-ip`}}", "WS1IP={{user `webserver-one-ip`}}", "WS2IP={{user `webserver-two-ip`}}", "WS3IP={{user `webserver-three-ip`}}", "REDIP={{user `redis-server-ip`}}"]
