...
sudo sed -i "s/.*bind-address.*/#bind-address = $MS2IP/" /etc/mysql/mariadb.conf.d/50-server.cnf 
sudo sed -i "s/.*server-id.*/server-id      =3/" /etc/mysql/mariadb.conf.d/50-server.cnf
...
mongosh --host 127.0.0.1 < ~/hajek/itmt-595/fullstack-app-code/mongodb/set-read-pref.js