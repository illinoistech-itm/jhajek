...
Using the routing table information, find each systems *default gateway* or *default route* for the interface that you identified in the first step of this exercise. On Windows, Mac, and FreeBSD use the command `netstat -r`, Linux no longer supports the netstat command, use the `ip route` command.
