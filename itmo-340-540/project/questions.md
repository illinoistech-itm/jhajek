...
Once you have retrieved an IP address via the DHCP process, you want to communicate between Computer A and Computer B to see if Computer B is on the network and responding to network traffic.

What would be the command line tool used to see if Computer B is responding to network traffic?  
i.

In the packet sent between Computer A to Computer B in the ITM network, fill in the values of the Src and Dest IP addresses
...
Internet Protocol Version 4
    ...
    Time to Live: 128
    Protocol: ICMP (1)
    Header Checksum: 0xd242 [validation disabled]
    [Header checksum status: Unverified]
    Source Address: XXX.XXX.XXX.XXX
    Destination Address: XXX.XXX.XXX.XXX

Before a ICMP packet can be sent, what protocol would Computer A use to resolve the MAC address of Computer B (assuming it doesn't have it cached)?  
i.
What layer of the 5 Layer Network Model does this protocl work at (hint it is a LAN protocol)?  
i.

Fill in the blank: Looking at the ARP (Address Resolution Protocol) protocol, it is used at the _____________ layer for resolving IP addresses to MAC addresses.  
i. 
Looking at this ARP Packet trace that was sent from Computer B to Computer A, what would be the values placed in the XXX place-holders 
...
====================================================================
Assume Computer B has an HTTP server. From Computer A assume you open a web browser and make a request to http://192.168.1.11

Using this WireShark trace fill in the appropriate XXX values
...
What is the name for the first three TCP packets that are exchanged before an HTTP connection is made.  
i.

Using this WireShark capture of the HTTP connection between Computer A and Computer B: 
...
List the HTTP version  
i.
List the HTTP method  
i.
List the Request URI  
i.
...
Based on the three networks detailed - create a forwarding table that includes prefix and output port by updating the table below.
...
Which Output Port Interface will your packet be forwarded to?  
i.
For the TCP packet returning the ACK from Computer B to Computer A, which Output Port Interface will your packet be forwarded to?  
i.

=============================================================================================
DNS

Looking at the snippets of these two DNS packet queries (DNS query and DNS reponse) replace the values.  Computer A is making an http GET request to http://fusion.cs.iit.edu.
