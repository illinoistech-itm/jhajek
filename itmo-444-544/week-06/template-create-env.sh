...
EC2IDS=$(aws ec2 run-instances --image-id $1 --instance-type $2 --key-name $3 --security-group-ids $4 --count $5 --placement $6 --user-data file://install-env.sh --query='Reservations[*].Instances[*].InstanceId' --filters Name=instance-state-name,Values=pending --no-cli-pager)
