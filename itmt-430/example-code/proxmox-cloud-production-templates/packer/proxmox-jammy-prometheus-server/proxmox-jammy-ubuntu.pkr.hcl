...
source "proxmox-iso" "jammy-prometheus-server-system41" {
  boot_command = [
        "e<wait>",
        "<down><down><down>",
...
##############################################################################
# Prometheus Template for system42
##############################################################################source "proxmox-iso" "jammy-prometheus-server-pve" {
source "proxmox-iso" "jammy-prometheus-server-system42" {  
  boot_command = [
        "e<wait>",
        "<down><down><down>",
        "<end><bs><bs><bs><bs><wait>",
...
build {
  sources = ["source.proxmox-iso.jammy-prometheus-server-system41","source.proxmox-iso.jammy-prometheus-server-system42"]
  ...
  ########################################################################################################################
  # Copy the prometheus service file to the template so that the instance can publish its own system metrics on the
  # metrics interface
  ########################################################################################################################

  provisioner "file" {
    source      = "../scripts/proxmox/jammy-services/prometheus.service"
    destination = "/home/vagrant/"
  }
  ...
  provisioner "file" {
    source      = "../scripts/proxmox/jammy-prometheus/prometheus.yml"
    destination = "/home/vagrant/"
  }
  ...
  ############################################################################################
  # Script to install Prometheus server and firewall dependencies
  #
  #############################################################################################
  
  provisioner "shell" {
    execute_command = "echo 'vagrant' | {{ .Vars }} sudo -E -S sh '{{ .Path }}'"
    scripts         = ["../scripts/proxmox/jammy-prometheus/post_install_prxmx_ubuntu_firewall-additions.sh",
                       "../scripts/proxmox/jammy-prometheus/post_install_prxmx_ubuntu_prometheus_server_install.sh"]
  }
}
