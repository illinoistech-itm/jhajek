...
source "proxmox-iso" "proxmox-jammy-grafana" {
  boot_command = [
        "e<wait>",
        "<down><down><down>",
  ...
  ssh_timeout              = "20m"
  template_description     = "A Packer template to create a Grafana/Graphite template for Ubuntu Jammy"
  vm_name                  = "${var.VMNAME}"
}

build {
  sources = ["source.proxmox-iso.proxmox-jammy-grafana"]
