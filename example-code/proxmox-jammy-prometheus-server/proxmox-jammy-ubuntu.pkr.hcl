...
source "proxmox-iso" "jammy-prometheus-server" {
  boot_command = [
        "e<wait>",
        "<down><down><down>",
...
build {
  sources = ["source.proxmox-iso.jammy-prometheus-server"]
  ...
  provisioner "file" {
    source      = "../scripts/proxmox/jammy-services/node_exporter.service"
    destination = "/home/vagrant/"
  }
