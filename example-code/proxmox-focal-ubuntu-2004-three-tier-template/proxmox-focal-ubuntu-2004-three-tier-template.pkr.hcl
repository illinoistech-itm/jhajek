...
source "proxmox-iso" "ws-template" {
  ...
  vm_name                  = "${var.VMNAME-WS}"
}

source "proxmox-iso" "lb-template" {
  boot_command = ["<enter><enter><f6><esc><wait> ", "autoinstall ds=nocloud-net;seedfrom=http://{{ .HTTPIP }}:{{ .HTTPPort }}/", "<enter><wait>"]
  boot_wait    = "5s"
  cores        = "${var.NUMBEROFCORES}"
  node         = "${var.NODENAME}"
...
source "proxmox-iso" "db-template" {
  boot_command = ["<enter><enter><f6><esc><wait> ", "autoinstall ds=nocloud-net;seedfrom=http://{{ .HTTPIP }}:{{ .HTTPPort }}/", "<enter><wait>"]
  boot_wait    = "5s"
  cores        = "${var.NUMBEROFCORES}"
  node         = "${var.NODENAME}"
  ...
  ssh_wait_timeout         = "1200s"
  template_description     = "A Packer template to create a Promox Template - Vanilla Ubuntu"
  vm_name                  = "${var.VMNAME-DB}"
}

build {
  sources = ["source.proxmox-iso.ws-template","source.proxmox-iso.lb-template","source.proxmox-iso.db-template"]
  ...
  # You can use an only block to apply a shell script to a single template
  ########################################################################################################################
  ...
  ########################################################################################################################
  # This will run a shell script only on a single template -- the default is on all templates created
  ########################################################################################################################

  #  provisioner "shell" {
  #    execute_command = "echo 'vagrant' | {{ .Vars }} sudo -E -S sh '{{ .Path }}'"
  #    scripts          = ["../path/to/your/shell/script.sh"]
  #    only             = ["proxmox-iso.lb-template"]
  #  }
