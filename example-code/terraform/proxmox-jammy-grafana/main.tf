...
resource "proxmox_vm_qemu" "grafana" {
  ...
  network {
    model  = "virtio"
    bridge = "vmbr0"
    macaddr = "04:9F:15:00:00:01"
  }

  network {
    model  = "virtio"
    bridge = "vmbr1"
    macaddr = "04:9F:15:00:01:00"
  }

  network {
    model  = "virtio"
    bridge = "vmbr2"
    macaddr = "04:9F:15:01:00:00"
  }
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo sed -i 's/0.0.0.0/${var.yourinitials}-vm${count.index}.service.consul/' /etc/systemd/system/node_exporter.service",
      "sudo systemctl daemon-reload",
...
output "proxmox_ip_address_default" {
  ...
  value = proxmox_vm_qemu.grafana.*.default_ipv4_address
}