...
spark = SparkSession.builder.appName("JRH convert 40.txt to csv").config( "spark.driver.host", "localhost" ).config(conf=conf).getOrCreate()
 
df = spark.read.csv('s3a://itmd521/40.txt')
...
splitDF.write.format("csv").mode("overwrite").option("header","true").save("s3a://itmd521/40.csv")